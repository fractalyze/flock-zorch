...
|------|-----------|-----------|
| software `fori_loop` (jax, in-prover) | 0.122 G mul/s | 270× below |
...
/home/jooman/fractalyze/zorch/.venv/bin/python optim/clmad/validate.py   # byte-match + bench
```
...
plugin uses CUDA 12.x and cannot emit `clmad`. To make the in-prover `field.mul`
(and the NTT, which is field-mul-bound) use `clmad`, EITHER:
...
Either way, the byte-match gate (`field_oracle_test.py`) stays the correctness oracle.
