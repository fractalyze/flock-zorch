...
| 24 | 71.3           | 66.4           | **1.07×** |
| 26 | 277.1          | 94.1           | **2.9×** |
| 28 | 1,167.1        | 185.7          | **6.3×** |

Ligerito re-measured 2026-07-16 on the device-native open (#479 one-device-program
recursive open + on-device query sampling, #104): ~3× faster than the 2026-06-30
baseline, crossover ≈27→≈24 (GPU wins from m=24), 6.3× by m=28.
...
**m ≈ 24 for Ligerito** (Keccak3) after the device-native open. Above the crossover
the GPU advantage keeps growing with m — Keccak3 **6.3×** by m=28.
...
2. **The CPU baseline is x86 *scalar*.** flock's NEON/parallel paths are
   ...
   built on a MacBook.
3. **The GPU only wins above the crossover** (m ≳ 24 BaseFold, ≳ 24 Ligerito). For
