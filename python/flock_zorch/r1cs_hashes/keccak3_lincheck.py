...
def _subkeccak_columns():
    ...
    for i in range(N_SUB):
        ...
        rt = (
            T_PACKED_BIT_BASE
            + (i * N_T + np.arange(N_T))[:, None] * STATE_BITS
            + _WLC[None, :]
        ).astype(np.int64)
        rows_t.append(rt)
...
class Keccak3LincheckCircuit:
    """The procedural 3-wide keccak lincheck walker.

    Ports flock `keccak3::KeccakLincheckCircuit`.
    """

    n_cols = K
    const_pin = Z_CONST  # shared const-wire pin column (lincheck.prove applies +β here)
    _sub_cols = [
        (_COL0[i], _COL24[i], _ROWS_T[i]) for i in range(N_SUB)
    ]  # host (test ref)
