...
def run():
    ...
    _lr, lc_zp, lc_claim = lincheck.prove(g["zlc"], None, None, x_ab, m, k_log, k_skip, ch=ch, circuit=csc)
...
def main() -> int:
    ...
    m, results = run()
    return report(results, f"blake3 LIGERITO full prove (R1csProofLigerito) vs flock prove_ligerito (m={m})")
