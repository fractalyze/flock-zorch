...
def _rand(n, seed):
    return fnp.asarray(
        np.random.default_rng(seed).integers(0, 2**64, size=(n, 2), dtype=np.uint64)
    )
...
def main():
    ...
    for log in (16, 18, 20):
        ...
        try:
            dt = _bench(fn, (d,), 20)
            print(
                f"  log_d={log:<2} {dt*1e3:8.2f} ms/transform  {n/dt/1e9:6.3f} G elem/s"
            )
        except Exception as e:  # noqa: BLE001
            print(
                f"  log_d={log:<2} {'OOM' if _oom(e) else type(e).__name__}: "
                f"{str(e)[:70]}"
            )
        del d
        gc.collect()
    ...
    for log in (16, 18, 20):
        ...
        print(
            f"  log={log:<2} build_eq {eq_ms:7.3f} ms "
            f"({n / (eq_ms / 1e3) / 1e9:5.2f} G elem/s)"
