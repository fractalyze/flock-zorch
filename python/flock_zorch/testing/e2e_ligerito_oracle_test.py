...
Run (regen: cargo run --release --example dump_e2e_ligerito -- 22 \
artifacts/e2e_ligerito_golden.bin):
...
def run():
    ...
    res = prover.prove_fast(
        g["z"], m, k_log, k_skip, a0, b0, g["zlc"], g["stmt"], cfg, domain=DOMAIN
    )
    ...
    got_mlv = np.array(
        [
            np.concatenate([_lanes(a).reshape(2), _lanes(b).reshape(2)])
    ...
    results.append(
        (
            ...
            got_mlv.shape == want_mlv.shape and np.array_equal(got_mlv, want_mlv),
        )
    )
    results.append(
        ("zc final_a", np.array_equal(_lanes(zc.final_a_eval).reshape(2), gzc["fa"]))
    )
    results.append(
        ("zc final_b", np.array_equal(_lanes(zc.final_b_eval).reshape(2), gzc["fb"]))
    )
    results.append(
        ("zc final_c", np.array_equal(_lanes(zc.final_c_eval).reshape(2), gzc["fc"]))
    )
    ...
    want_lcr = (
        np.array([np.concatenate([a, b]) for a, b in g["lc"]["rounds"]])
        if g["lc"]["rounds"]
        else np.zeros((0, 4), np.uint64)
    )
    results.append(
    ...
    results.append(
        (
            "claim ab.value",
            np.array_equal(_lanes(res.claim_ab_value).reshape(2), g["ab_v"]),
        )
    )
    results.append(
        (
            "claim c.value",
            np.array_equal(_lanes(res.claim_c_value).reshape(2), g["c_v"]),
        )
    )
...
def main() -> int:
    ...
    return report(
        results,
        f"e2e LIGERITO fused prove (prove_fast) vs flock "
        f"prove_fast_ligerito (identity m={m})",
    )
