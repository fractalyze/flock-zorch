...
def run():
    ...
    in_vals, out_vals = chain.fold_in_out(
        g["z"], k_log, tau_pos, meta["input_byte_off"], meta["output_byte_off"]
    )
    ...
    got_sr = (
        np.array(
            [
                np.concatenate(
                    [ghash.to_lanes(e1).reshape(2), ghash.to_lanes(ei).reshape(2)]
                )
                for e1, ei in sh_rounds
            ]
        )
        if sh_rounds
    ...
    want_sr = (
        np.array([np.concatenate([e1, ei]) for e1, ei in g["shift"]["rounds"]])
        if g["shift"]["rounds"]
        else np.zeros((0, 4), np.uint64)
    )
    results.append(
        (
            "shift rounds",
            got_sr.shape == want_sr.shape and np.array_equal(got_sr, want_sr),
        )
    )
    results.append(
        (
            "shift g_at_point",
            np.array_equal(
                ghash.to_lanes(sh_claims["value"]).reshape(2), g["shift"]["g_at_point"]
            ),
        )
    )
...
def main() -> int:
    ...
    return report(
        results,
        f"keccak CHAIN prove (ChainProofLigerito: zc+lc+shift+mixed open) "
