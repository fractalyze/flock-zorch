...
Run (regen: cargo run --release --example dump_keccak3_ligerito -- 49 \
artifacts/keccak3_ligerito_golden.bin):
...
def substitute_device_witness(g):
    ...
    keccak3 writes whole lanes into aligned slots, so all three input states sit
    verbatim in the golden's own z prefix and `extract_inputs` reads them back —
    no extra fixture exists to go stale.
...
def run(device_witness=False):
    ...
    ch = Sha256Challenger(b"flock-keccak3-lig-v0")
    ...
    # Stage W: the keccak3 walker port —
    # Keccak3LincheckCircuit.fold_alpha_batched (standalone)
    for i, pb in enumerate(g["probes"]):
        comb = ghash.to_lanes(
            circ.fold_alpha_batched(
                ghash.to_ghash(pb["alpha"]), ghash.to_ghash(pb["eq"])
            )
        )
        results.append(
            (f"walker probe {i} (fold_alpha_batched)", np.array_equal(comb, pb["comb"]))
        )
    return m, results
