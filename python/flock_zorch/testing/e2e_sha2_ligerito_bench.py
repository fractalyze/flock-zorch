...
def main():
    ...
    m, lir, lbs = meta["m"], meta["lir"], meta["lbs"]; k_log, k_skip = meta["k_log"], meta["k_skip"]; ir = k_log - k_skip
    ...
    z, stmt = g["z"], g["stmt"]
    print(f"device {jax.devices()[0]} | mul {'clmad' if MUL is not field.mul else 'software'} | sha2-ligerito m={m}")
    def prove_once():
        ...
        _r, _zp, lcc, _zv = lincheck.prove(g["zlc"], None, None, x_ab, m, k_log, k_skip, mul=MUL, ch=ch, capture=True, circuit=csc)
        ab = np.concatenate([lcc["r_inner_rest"], x_ab["x_outer"]], axis=0)
        cc = np.concatenate([zc["r_rest"][:ir], zc["r_rest"][ir:]], axis=0)
        return prover.open_batch_ligerito(cfg, z, codeword, tree, [ab, cc], ch, mul=MUL, use_host_sha=True)
