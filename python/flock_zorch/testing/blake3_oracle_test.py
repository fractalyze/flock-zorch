"""GPU BLAKE3 R1CS prover byte gate vs flock `prove` (BaseFold), real blake3 R1CS.

Ingests flock's real blake3 instance (dump_blake3.rs: Blake3Setup, K_LOG=14,
const_pin=512, USEFUL_BITS=15409, inner_rest=8) — witness gen + the R1CS are
host setup, not the GPU target — and replays flock-zorch's prover phases on ONE
shared challenger, byte-comparing each stage. BLAKE3 mirrors sha2: its a_0/b_0
are populated sparse matrices, folded by the generic `CscCircuit` (no
blake3-specific Python). Stages:
  A: commit root          (PCS commit on the real packed witness)
  B: zerocheck proof+claim (real a=A·z, b=B·z, c=z; useful_bits padding)
  C: lincheck CSC fold     (k=16384, const_pin=512, inner_rest=8)
  D: batched dual-claim open (ab from lincheck, c from zerocheck)
...
def load():
    rd = R((ART / "blake3_golden.bin").read_bytes())
    assert bytes(rd.take(8)) == b"FLKBLK01", "bad magic"
...
def run():
    ...
    # Stage C: CSC sparse lincheck (k=16384, const_pin=512, inner_rest=8) — shared challenger
    k = 1 << g["meta"]["k_log"]
...
def main() -> int:
    ...
    print(f"blake3 FULL prove (commit+zerocheck+CSC lincheck+batched open) vs flock BaseFold "
