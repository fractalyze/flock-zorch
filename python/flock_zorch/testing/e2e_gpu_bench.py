...
def bench(m):
    ...
    t_merkle = _best(lambda: merkle.merkle_tree(cw_np.reshape(n_pos_code, num_ntts * 2).view(np.uint8)), n=2)
    init_tree = merkle.merkle_tree(cw_np.reshape(n_pos_code, num_ntts * 2).view(np.uint8))
