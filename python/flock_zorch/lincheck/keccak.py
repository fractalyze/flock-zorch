...
def _gather_xor(vals, map_T):
    ...
    `vals` is uint64 [.,2] lanes — the walker's XOR/gather/scatter/select stay in
    the lane domain (ghash reductions/selects/gathers lower ~170x slower on GPU);
    only the final α-combine is a field multiply."""
...
def _accumulate_subkeccak(eq, col_state0, col_state24, rows_t):
    ...
    # vector; it masks chi via a lane-domain 0/1 multiply (no field mul).
...
def _fold_walker_kernel(eq, alpha, sub_cols, z_const):
    ...
    multiply. `eq` is (n_cols, 2) uint64 lanes; `alpha` native ghash; `sub_cols` a
    list of (col_state0, col_state24, rows_t) device index arrays; `z_const` a
    static int column. The accumulation stays in lanes; only the α-combine is a
    field multiply, so the returned comb is native ghash."""
    n_cols = eq.shape[0]
    ...
    return alpha * ghash.to_ghash(comb_a) + ghash.to_ghash(comb_b)  # α·comb_a ⊕ comb_b (field mul)
