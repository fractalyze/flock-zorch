//! Golden-fixture dumper for flock's REAL BLAKE3 R1CS prover (BaseFold path) —
//! the gate for flock-zorch's GPU blake3 prover.
//!
//! Builds the blake3 compression R1CS (`Blake3Setup`, K_LOG=14, K_SKIP=6,
//! USEFUL_BITS=15409, const_pin=Z_CONST_POS=512), generates a real multi-block
//! witness (one BLAKE3 compression per k-block), and runs flock's top-level
//! `prove` (BaseFold: commit→bind→zerocheck→lincheck→batched dual-claim open).
//!
//! BLAKE3 mirrors sha2 exactly: its `a_0`/`b_0` are populated sparse matrices
//! (`build_matrices`), folded by the generic CSC lincheck circuit — so the GPU
//! port reuses the same generic `CscCircuit` + staged prover as sha2 (no
//! blake3-specific Python). Dumps everything the GPU prover needs to INGEST
//! (witness gen + the R1CS are host setup, ~2% of prove, not the GPU target) +
//! the full R1csProof to gate:
...
//!
//! Usage: `cargo run --release --example dump_blake3 -- [n_compressions] [out]`
...
fn main() {
    ...
    let out = a.next().unwrap_or_else(|| "artifacts/blake3_golden.bin".to_string());
    ...
    // Real witness: one deterministic BLAKE3 compression per k-block. Any inputs
    // produce a satisfying witness (the R1CS encodes the compression relation,
    // not a target digest) — mirrors dump_sha2's random-compression approach.
    ...
    let z = blake3::generate_witness(&blocks, n_blocks_log);
    assert!(r1cs.satisfies(&z), "blake3 witness must satisfy the R1CS");
    ...
    // ---- dump ----
    let mut b = Vec::new();
    b.extend_from_slice(b"FLKBLK01");
    ...
    // R1csProof (same layout as dump_e2e / dump_sha2)
    ...
    println!("dumped blake3 e2e n_comp={n_comp} m={m} k_log={} k_skip={} ub={} const_pin={:?} lbs={} | \
