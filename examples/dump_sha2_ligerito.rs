...
//!
//! Usage: `cargo run --release --example dump_sha2_ligerito -- [n_comp] [out]`
...
fn main() {
    ...
    let n_comp: usize = a.next().and_then(|s| s.parse().ok()).unwrap_or(128);
    let out = a.next().unwrap_or_else(|| "artifacts/sha2_ligerito_golden.bin".to_string());
    ...
    println!("dumped sha2-ligerito n_comp={n_comp} m={m} log_n={log_n} | R={} initial_k={} | \
